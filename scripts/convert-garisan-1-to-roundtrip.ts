...
async function convertGarisan1ToRoundtrip() {
  console.log('🚀 가리산 1번 코스 왕복 경로 변환 시작\n');
  console.log('='.repeat(80));
  console.log('작업 내용:');
  console.log('  1. 경로 역순 변경 (낮은 곳 → 정상 → 낮은 곳)');
  console.log('  2. 왕복 경로 생성 (역순 경로 추가)');
  ...
  if (error || !trail) {
    console.error('❌ 가리산 1번 코스 정방향을 찾을 수 없습니다:', error?.message);
    return;
  }
  ...
  // 2. 왕복 경로 생성: 원본 역순 + 다시 역순(처음~끝-1)
  // 정상에서 다시 내려오는 경로 추가 (마지막 지점 제외하고 역순으로 추가)
  ...
  console.log('✅ 업데이트 완료!\n');
  console.log('='.repeat(80));
  console.log('✨ 가리산 1번 코스 왕복 경로 변환 완료!');
  console.log('='.repeat(80));
  console.log('\n💡 경로 구조:');
  console.log('   출발(375m) → 정상(908m) → 도착(375m)');
  console.log('   완전한 왕복 코스로 변환되었습니다!');
}

convertGarisan1ToRoundtrip().then(() => process.exit(0));
