...
async function fixGarisan2Course() {
  console.log('🚀 가리산 2번 코스 수정 시작\n');
  console.log('='.repeat(80));
  console.log('작업 내용:');
  console.log('  1. 출발지점을 도착지점으로 변경 (순환 경로)');
  ...
  if (error || !trail) {
    console.error('❌ 가리산 2번 코스를 찾을 수 없습니다:', error?.message);
    return;
  }
  ...
  console.log('✅ 업데이트 완료!\n');
  console.log('='.repeat(80));
  console.log('✨ 가리산 2번 코스 수정 완료!');
  console.log('='.repeat(80));
}

fixGarisan2Course().then(() => process.exit(0));
