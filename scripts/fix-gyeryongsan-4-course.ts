...
async function fixGyeryongsan4Course() {
  console.log('🚀 계룡산 4번 코스 수정 시작\n');
  ...
  if (error || !trail) {
    console.error('❌ 계룡산 4번 코스를 찾을 수 없습니다:', error?.message);
    return;
  }
  ...
  console.log('✅ 업데이트 완료!\n');
  console.log('='.repeat(80));
  console.log('✨ 계룡산 4번 코스 수정 완료!');
  console.log('='.repeat(80));
}

fixGyeryongsan4Course().then(() => process.exit(0));
